...
def tongues(code):
    vowels_in = 'aiyeou'
    consonants_in = 'bkxznhdcwgpvjqtsrlmf'
    vowels_out = vowels_in[3:] + vowels_in[:3]
    consonants_out = consonants_in[10:] + consonants_in[:10]
    word_in= vowels_in + consonants_in
    word_out = vowels_out + consonants_out
    result = str.maketrans(word_in + word_in.upper(), word_out + word_out.upper())
    return code.translate(result)